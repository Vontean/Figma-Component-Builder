...
- 输出组件范围、Token/Style 复用或新增项、矩阵状态、文档写入范围和待决策差异。

完成条件：组件范围和迁移边界明确；矩阵已有可执行来源，或候选矩阵已获设计确认。
...
- 按确认的 Anatomy 结构决策和关联 reference 创建图层、公开 API、组件角色和交互热区；constraints、swap 和部件抽取在建层时完成。
- 将对应 `Component Token` 或 Style 绑定到图层；建立 `Component Property` 与子图层/实例的实际关联。

完成条件：每个目标 `Component Set` 的 variant、公开 API、结构与绑定均已验证。
